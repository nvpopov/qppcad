...
TEST_CASE("history stream test") {
  ...
  SECTION ("testing temporary objects") {
    ...
    REQUIRE(hs_iarray->get_hs_children_count() == 2);
    REQUIRE(hs_root->get_cur_epoch() == 2);
    REQUIRE(hs_int1.use_count() == 3);

    hs_int1->set_doctype(hs_doc_type_e::hs_doc_persistent);
    REQUIRE(hs_root->get_cur_epoch() == 3);
    REQUIRE(hs_iarray->get_hs_children_count() == 2);
...
//    std::cout << "EP2: "<< std::endl << hs_xg->print_epoch_info(2) << std::endl;
  ...
  SECTION ("xgeom proxy - selection tests") {
