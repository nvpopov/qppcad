...
void mesh_t::bind_data() {
  ...
  astate->tlog("[MESH] Pre binding data to mesh[vao={}, vbo={}, nbo={}, vio={}]",
               vao, vbo, nbo, vio);

  glapi->glBindVertexArray(vao);

  glapi->glBindBuffer(GL_ARRAY_BUFFER, vbo);
  glapi->glBufferData(GL_ARRAY_BUFFER, vertecies.size()*sizeof(float),&vertecies[0],GL_STATIC_DRAW);
  glapi->glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, nullptr);
  glapi->glEnableVertexAttribArray(0);

  glapi->glBindBuffer(GL_ARRAY_BUFFER, nbo);
  glapi->glBufferData(GL_ARRAY_BUFFER, normals.size()*sizeof(float), &normals[0], GL_STATIC_DRAW);
  glapi->glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 0, nullptr);
  glapi->glEnableVertexAttribArray(1);

  glapi->glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, vio);
  glapi->glBufferData(GL_ELEMENT_ARRAY_BUFFER,indices.size()*sizeof(int),&indices[0], GL_STATIC_DRAW);
  glapi->glBindVertexArray(0);

  astate->tlog("[MESH] Binding data to mesh[vao={}, vbo={}, nbo={}, vio={}] with vs={}, ns={}, is={}",
               vao, vbo, nbo, vio,
               vertecies.size(),
               normals.size(),
               indices.size());
