  ...
  for (auto target_state : target_states) {
    ...
    for (auto ws : astate->ws_mgr->m_ws)
      for (size_t i = 0; i < ws->num_items(); i++) {
        ...
        if (volume_view_t *vv = ws_item->cast_as<volume_view_t>(); vv != nullptr) {
          ...
          for (auto vol_rec : vv->m_volumes) { // begin iterate over volume records
            ...
            if (vol_rec->m_state_id == target_state) {
              ...
              auto sp_vol_v = std::static_pointer_cast<volume_view_t>(ws_item);
...
void ccd_view_tools_t::build_volume_for_root(ccd_view_t *ccd, volume_view_t *target_vv,
                                             size_t root_id, float min_amplitude) {
