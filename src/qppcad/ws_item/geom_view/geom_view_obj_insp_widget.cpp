...
void geom_view_obj_insp_widget_t::construct_mod_tab() {
  ...
  m_tm_grp_op_lt->setContentsMargins(5, 0, 5, 0);
...
void geom_view_obj_insp_widget_t::bind_angle_msr_tab() {
  ...
  if (b_al) {
      ...
      if (cur_msr < b_al->m_measure->m_angle_recs.size() &&
          ...
          m_tms_angle_enabled->bind_value(&rec.m_show);
          m_tms_angle_enabled->setEnabled(true);

          m_tms_angle_order->bind_value(&rec.m_order);
...
void geom_view_obj_insp_widget_t::update_mod_tab() {
  ...
  if (b_al) {
      ...
      if (b_al->m_parent_ws &&
          ...
          m_tm_grp_op_flip_a_p->setEnabled(b_al->m_geom->DIM > 0);
          m_tm_grp_op_flip_b_p->setEnabled(b_al->m_geom->DIM > 1);
          m_tm_grp_op_flip_c_p->setEnabled(b_al->m_geom->DIM > 2);
          m_tm_grp_op_flip_a_n->setEnabled(b_al->m_geom->DIM > 0);
          m_tm_grp_op_flip_b_n->setEnabled(b_al->m_geom->DIM > 1);
          m_tm_grp_op_flip_c_n->setEnabled(b_al->m_geom->DIM > 2);
...
void geom_view_obj_insp_widget_t::disp_switch_cur_changed(int index) {
  ...
  m_gb_periodic_related_render->setVisible(index == 0 && b_al && b_al->m_geom->DIM > 0);
...
void geom_view_obj_insp_widget_t::cell_changed() {
  ...
  if (b_al) {
      ...
      if (b_al->m_geom->DIM > 0) {
          ...
          m_tg_gb_cell_tbl->setRowCount(b_al->m_geom->DIM);
...
void geom_view_obj_insp_widget_t::mod_translate_selected_atoms_clicked() {
  ...
  if (b_al) {
      ...
      if (m_tm_translate_coord_type->currentIndex() == 1 && b_al->m_geom->DIM == 3) {
...
void geom_view_obj_insp_widget_t::mod_bc_rot_angle_type_change(int new_angle_type) {
  ...
  if (new_angle_type == 0) m_tm_bc_rot_angle->setSingleStep(0.5);
  else m_tm_bc_rot_angle->setSingleStep(0.01);
