...
traj_hl_t::traj_hl_t() : ws_item_t() {
  ...
  m_traj_style.set_value(traj_hl_style_e::traj_hl_style_points); add_hs_child(&m_traj_style);
...
void traj_hl_t::render() {
  ...
  if (m_is_visible.get_value() && !m_need_to_rebuild && m_leader) {
    ...
    if (traj_style == traj_hl_style_e::traj_hl_style_points
        || traj_style == traj_hl_style_e::traj_hl_style_lines
        || traj_style == traj_hl_style_e::traj_hl_style_lines_loop) {
      ...
      if (traj_style == traj_hl_style_e:: traj_hl_style_spheres && m_binded_gv
