...
PYBIND11_EMBEDDED_MODULE(workspace_stuff, m) {
  ...
  py_ws_item_t.def_readwrite("m_name", &ws_item_t::m_name);
  py_ws_item_t.def("get_cnt_count", &ws_item_t::get_content_count);
  py_ws_item_t.def("__repr__", [](ws_item_t &wsi){return wsi.m_name;});
  py_ws_item_t.def("__str__", [](ws_item_t &wsi){return wsi.m_name;});
