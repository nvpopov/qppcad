...
void structure_similarity_widget_t::compute_structure_similarity(ws_item_t *g1, ws_item_t *g2) {
  ...
  if (gv1)
    m_str_sim_out->insertPlainText(" Yes\n");
  else
    m_str_sim_out->insertPlainText(" No\n");
...
void structure_similarity_widget_t::set_out_table_data(geom_view_t *g1, geom_view_t *g2,
  ...
  if (_dp_l > 0.001f)
    dp_lr->setBackgroundColor(Qt::red);
  ...
  if (abs(_dp[0]) > 0.001f)
    dp_x->setBackgroundColor(Qt::red);
  ...
  if (abs(_dp[1]) > 0.001f)
    dp_y->setBackgroundColor(Qt::red);
  ...
  if (abs(_dp[2]) > 0.001f)
    dp_z->setBackgroundColor(Qt::red);
...
void str_sim_ws_item_rec_t::cmb_it_changed(int idx) {
  ...
  if (ws_idx >= 0
    ...
    auto ws_item = wsm->m_ws[ws_idx]->m_ws_items.get_hs_child_as_array(idx);
    if (ws_item) {
      ...
      astate->tlog("Invalid ws_idx={} in str_sim_ws_item_rec_t::cmb_it_changed", idx);
    }
  }
